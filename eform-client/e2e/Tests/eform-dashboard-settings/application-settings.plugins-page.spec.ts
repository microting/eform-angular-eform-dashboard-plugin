...
describe('Application settings page - site header section', function () {
  ...
  it('should go to plugin settings page', function () {
    ...
    expect(plugin.name).equal('Microting eForm Dashboard Plugin');
  ...
  it('should activate the plugin', function () {
    ...
    expect(pluginToFind.name).equal('Microting eForm Dashboard Plugin');
    expect(pluginToFind.version).equal('1.0.0.0');
    $(`//*[contains(text(), 'eForm Dashboard')]`).waitForDisplayed({timeout: 30000});
  });
});
