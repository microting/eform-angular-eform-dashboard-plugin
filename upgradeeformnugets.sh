...
if (( "$GIT_STATUS" > 0 )); then
	git pull
	cd eFormAPI/Plugins/eFormDashboard.Pn/eFormDashboard.Pn

	dotnet add eFormDashboard.Pn.csproj package Microting.eForm
	dotnet add eFormDashboard.Pn.csproj package Microting.eFormApi.BasePn
	dotnet add eFormDashboard.Pn.csproj package Microting.eFormDashboardBase
	...
	EFORM_PLUGINBASE_VERSION=`dotnet list package | grep 'Microting.eFormDashboardBase' | cut -c64-71`
	COMMIT_MESSAGE="Updating"$'\n'"- Microting.eForm to ${EFORM_VERSION}"$'\n'"- Microting.eFormApi.BasePn to ${EFORM_BASEPN_VERSION}"$'\n'"- Microting.eFormDashboardBase to ${EFORM_PLUGINBASE_VERSION}"
